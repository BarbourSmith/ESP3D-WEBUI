...
function gnComputeFAndJ(params, meas, n, computeAbs = false) {
    const x1 = params[0], x2 = params[1], y2 = params[2], x3 = params[3], y3 = params[4];
    const f = new Array(2 * n).fill(0);
    const J = Array.from({length: 2 * n}, () => new Array(GN_N_PARAMS).fill(0));
    let abs_res_br = null;
    let abs_res_bl = null;
    if (computeAbs) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (Math.abs(x1) < 1e-9) {
            f[2 * i] = f[2 * i + 1] = 1e18;
            for (let k = 0; k < GN_N_PARAMS; k++) { J[2 * i][k] = J[2 * i + 1][k] = 0; }
...
function gnComputeThreshold(allAbsRes) {
...
function gnRunSolver(initialParams, meas, nn) {
    let params = [...initialParams];
    let JTJ = Array.from({length: GN_N_PARAMS}, () => new Array(GN_N_PARAMS).fill(0));
    let JTf = new Array(GN_N_PARAMS).fill(0);
    let delta = new Array(GN_N_PARAMS).fill(0);
    let aug = Array.from({length: GN_N_PARAMS}, () => new Array(GN_N_PARAMS + 1).fill(0));

    let converged = false;
    for (let iter = 0; iter < GN_MAX_ITER; iter++) {
        const {f, J} = gnComputeFAndJ(params, meas, nn);

        // Compute J^T J and J^T f
        for (let j = 0; j < GN_N_PARAMS; j++) {
            JTf[j] = 0;
            for (let k = 0; k < GN_N_PARAMS; k++) JTJ[j][k] = 0;
        }
        for (let i = 0; i < 2 * nn; i++) {
            for (let j = 0; j < GN_N_PARAMS; j++) {
                JTf[j] += J[i][j] * f[i];
                for (let k = 0; k < GN_N_PARAMS; k++) {
                    JTJ[j][k] += J[i][j] * J[i][k];
                }
            }
        }
        ...
        // Augment
        for (let j = 0; j < GN_N_PARAMS; j++) {
            for (let k = 0; k < GN_N_PARAMS; k++) aug[j][k] = JTJ[j][k];
            aug[j][GN_N_PARAMS] = -JTf[j];
        }

        if (!gnSolveGauss(aug, delta)) {
            return null;
        }
    ...
    const {f: final_f, J: final_J} = gnComputeFAndJ(params, meas, nn);
    let residual = 0;
    let sum_sq = 0;
    for (let i = 0; i < 2 * nn; i++) {
    ...
    // Covariance
    const dof = 2 * nn - GN_N_PARAMS;
    const sigma2 = (dof > 0) ? sum_sq / dof : 0;

    // Recompute JTJ
    const JTJ_final = Array.from({length: GN_N_PARAMS}, () => new Array(GN_N_PARAMS).fill(0));
    for (let i = 0; i < 2 * nn; i++) {
        for (let j = 0; j < GN_N_PARAMS; j++) {
            for (let k = 0; k < GN_N_PARAMS; k++) {
    ...
    const inv_JTJ = gnInvert5x5(JTJ_final);
    if (inv_JTJ === null) return null;

    const stds = new Array(GN_N_PARAMS).fill(0);
    for (let j = 0; j < GN_N_PARAMS; j++) {
...
/**
 * Gauss-Newton Least Squares calibration algorithm
 * This function has the same signature as findMaxFitness for compatibility
 * @param {Array} measurements - Array of measurement objects with {tl, tr, br, bl}
 * @returns {Object} - Calibration result
 */
function findFitnessGaussNewtonLeastSquares(measurements) {
    const messagesBox = document.getElementById('messages');
    ...
    // Full fit
    const fullResult = gnRunSolver(initialParams, measurements, N);
    if (fullResult === null) {
    ...
    // Detect outliers
    const outlierResult = gnComputeFAndJ(fullResult.params, measurements, N, true);
    ...
    const threshold = gnComputeThreshold(allAbsRes);
    ...
    if (outlierIndices.length > 0) {
        ...
        cleanResult = gnRunSolver(fullResult.params, goodMeasurements, goodMeasurements.length);
        if (cleanResult !== null) {
